...
function del(config, auth, className, objectId) {
  ...
  return Promise.resolve().then(() => {
    ...
    return Promise.resolve({});
      ...
      // #1 !!!!
      // This entire block has been added to provide a role check mechanism
  
	  // perform a check for user auth roles
	  return auth.getUserRoles().then((roles) => {
	    
	    // additionally push user id
	    auth.userRoles = auth.user.id;
	
		// set user roles
		auth.userRoles = roles;
    ...
    if (!auth.isMaster) {
      ...
      if (auth.user) {
          ...
          // #2 !!!!!
         // Updated check for ROLES
         options.acl.push(auth.userRoles);
